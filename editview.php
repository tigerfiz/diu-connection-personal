...
$select_quary = "SELECT * FROM `answerdata` WHERE id='$edit_value'";
$select_submit = $con->query($select_quary);
$select_result = $select_submit->fetch_assoc();
$select_result['answer'];

if (isset($_POST['editsubmitbtn'])) {
    $update_value = $_GET['id'];
    $editform =  checker($_POST['editform']);
    
    $questionkey = $select_result['questionkey'];
    $edit_quary = "UPDATE `answerdata` SET `answer`='$editform' WHERE id= $update_value";
    ...
    header('Location:view.php?id=' . $questionkey);
}
